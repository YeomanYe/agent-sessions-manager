...
> **状态**: Phase 1 MVP shipped 2026-05-28(在原 node-scripts 仓库) →
>          2026-05-29 拆出独立 monorepo `agent-sessions-manager`(本仓库)
> **创建**: 2026-05-26
> **修订**: 2026-05-28(v2 — 重大决策更新,见 §3) / 2026-05-29(v3 — 拆出独立仓库)
> **目标位置**: `packages/cli/src/ + packages/core/src/`
> **关联源资料**:
...
| 它是什么 | 它不是什么 |
|---|---|
| monorepo (pnpm workspace) 里的 CLI tool + core lib | 一个新 skill |
...
配置文件:`local/skill-recall-config.yaml`(local/ 是 git-ignored)
环境变量:`.env`(git-ignored,放 `MINIMAX_API_KEY`)
...
```bash
# 1. 在 .env (git-ignored) 设置
echo "MINIMAX_API_KEY=sk-xxxxx" >> ~/Documents/projects/agent-sessions-manager/.env
...
mkdir -p ~/Documents/projects/agent-sessions-manager/src/skill-recall/{types,loader,source,cache,storage,extractors,detectors,llm,reports}

# 5. 起 config 示例
mkdir -p ~/Documents/projects/agent-sessions-manager/local
# 写 skill-recall-config.yaml(从本 SPEC §8 复制)

# 6. 环境变量
echo "MINIMAX_API_KEY=" >> ~/Documents/projects/agent-sessions-manager/.env  # 然后手填真 key
```
